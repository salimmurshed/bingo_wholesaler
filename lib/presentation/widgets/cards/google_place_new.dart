...
import '../../../const/app_styles/app_input_styles.dart';
import '../../../const/app_styles/app_text_styles.dart';

class PlacesAutocompleteWidget<T> extends StatefulWidget {
  final String apiKey;
  ...
  ValueChanged<Prediction>? onchange;
...
class _PlacesAutocompleteOverlayState extends PlacesAutocompleteState {
  ...
  Widget build(BuildContext context) {
    ...
    if (_searching) {
      ...
      body = Material(
        color: theme.dialogBackgroundColor,
        child: SizedBox(), //widget.logo ?? PoweredByGoogleImage(),
        borderRadius: BorderRadius.only(
...
abstract class PlacesAutocompleteState extends State<PlacesAutocompleteWidget> {
  ...
  void initState() {
    super.initState();
    if (widget.getController!.text.isNotEmpty) {
      _queryTextController =
          TextEditingController(text: widget.getController!.text);
    } else {
    ...
    // _queryTextController!.text = widget.getController!.text;
